...
import crnn.ocr as ocr 
#import crnn.nets as crnn
...
class Network(nn.Module):
  ...
  def _add_score_summary(self, key, tensor):
    return tb.summary.histogram('SCORE/' + key + '/scores', 0, bins='auto')
  ...
  def _crop_pool_layer_extend(self, bottom, rois, max_pool=True, from_raw=False, crnn_h = 0, crnn_w = 50):
    ...
    if crnn_h>0:
        grid = F.affine_grid(theta, torch.Size((rois.size(0), 1, crnn_h, crnn_w)))
  ...
  def _crop_pool_layer(self, bottom, rois, max_pool=True, from_raw=False, crnn_h = 0, crnn_w = 50):
    # implement it using stn
    # box to affine
    # input (x1,y1,x2,y2)
    """
  ...
  def _proposal_target_layer(self, rois, roi_scores):
    rois, roi_scores, labels,texts,pair, bbox_targets, bbox_inside_weights, bbox_outside_weights = \
      proposal_target_layer(
      rois, roi_scores, self._gt_boxes, self._gt_texts, self._gt_pair, self._num_classes)

    self._proposal_targets['rois'] = rois
    self._proposal_targets['labels'] = labels.long()
    self._proposal_targets['texts'] = texts
    self._proposal_targets['pair'] = pair
    self._proposal_targets['bbox_targets'] = bbox_targets
    self._proposal_targets['bbox_inside_weights'] = bbox_inside_weights
    self._proposal_targets['bbox_outside_weights'] = bbox_outside_weights

    for k in self._proposal_targets.keys():
      if (k != 'texts') and (k != 'pair'):
  ...
  def _add_losses(self, sigma_rpn=3.0):
    ...
    # Pair-matching loss
    loss_pair=Variable(torch.FloatTensor([0])).cuda()    

    # CRNN, text loss
    text_pred = self._predictions['text_pred'] # TBA
    text_targets = self._proposal_targets['texts']
    loss_text = self._ctc_loss(label, text_pred, text_targets).cuda()

    self._losses['cross_entropy'] = cross_entropy
    self._losses['loss_box'] = loss_box
    self._losses['rpn_cross_entropy'] = rpn_cross_entropy
    self._losses['rpn_loss_box'] = rpn_loss_box
    self._losses['loss_text'] = loss_text
    self._losses['loss_pair'] = loss_pair

    loss = cross_entropy + loss_box + rpn_cross_entropy + rpn_loss_box + self._alp_pair*loss_pair+ self._alp_text*loss_text
  ...
  def _ctc_loss(self, label, text_preds, text_targets):
    inds_text = Variable(((label.data == 1)+(label.data==2)+(label.data==2)+(label.data==4)+(label.data==5)).nonzero().view(-1))

    if inds_text.nelement() == 0:
        return Variable(torch.FloatTensor([0])).cuda()    
    text_preds_filtered = text_preds.index_select(1,inds_text)
    text_targets_filtered = tuple([ ''.join(list(filter(lambda ch: ch in alphabet, text_targets[i]))) for i in inds_text.data])#text_targets.index_select(0,inds_text)
    ...
    pred_size = Variable(torch.IntTensor( [text_preds_filtered.size(0)] * b ))
  ...
  def _stn_crnn(self, pool_crnn):
    text_crnn = self.crnn_net(pool_crnn)
    self._predictions["text_pred"] = text_crnn 
    return text_crnn
  ...
  def create_architecture(self, num_classes, tag=None,
                          anchor_scales=(8, 16, 32), anchor_ratios=(0.5, 1, 2), test_mode=False):
    self._tag = tag
    ...
    print('number of anchors = ',self._num_anchors)
    print(__file__)

    assert tag != None

    # Initialize layers
    self._init_modules(test_mode)

  def _init_modules(self, test_mode=False):
    ...
    # crnn
    self.crnn_net = ocr.ocrModel(nc=3,nclass=len(alphabet)+1,nh=64) # if input is original image
    #self.crnn_net = crnn.CRNN(nc=3,nclass=len(alphabet),nh=256) # if input is original image

    self.init_weights()
    '''
    if not test_mode :
      self.cls_score_net = nn.DataParallel(self.cls_score_net)
      self.bbox_pred_net = nn.DataParallel(self.bbox_pred_net)
      self.crnn_net = nn.DataParallel(self.crnn_net)
    '''
    #self.crnn_net = crnn.CRNN(nc=512,nclass=len(alphabet),nh=256) # if input is feature map
  ...
  def _predict(self):
    ...
    #pool_crnn = self._crop_pool_layer(net_conv, rois, from_raw=False, max_pool=False, crnn_h=32, crnn_w=50)
    pool_crnn = self._crop_pool_layer(self._image, rois, from_raw=True, max_pool=False, crnn_h=30, crnn_w=100)
    #print(pool_crnn.size())
    if self._mode == 'TRAIN':
      torch.backends.cudnn.benchmark = True # benchmark because now the input size are fixed
    fc7 = self._head_to_tail(pool5)
    cls_prob, bbox_pred = self._region_classification(fc7)

    # from roi_pool layer to predicted texts
    text_crnn = self._stn_crnn(pool_crnn)
  ...
  def forward(self, image, im_info, gt_boxes=None, gt_texts=None, gt_pair=None, mode='TRAIN'):
    #print(gt_boxes)
    #print("image")
    #print(image.shape)
    ...
    self._gt_texts = gt_texts 
    self._gt_pair = gt_pair 
  ...
  def init_weights(self):
    ...
    def weights_init(m):
        ...
        if classname.find('Conv') != -1:
            ...
            m.bias.data.fill_(0)
    self.crnn_net.apply(weights_init)

    #normal_init(self.crnn_net.cnn, 0, 0.01, cfg.TRAIN.TRUNCATED)
  ...
  def test_image(self, image, im_info):
    ...
    cls_score, cls_prob, bbox_pred, rois, text_pred = self._predictions["cls_score"].data.cpu().numpy(), \
                                                     self._predictions['cls_prob'].data.cpu().numpy(), \
                                                     self._predictions['bbox_pred'].data.cpu().numpy(), \
                                                     self._predictions['rois'].data.cpu().numpy(), \
                                                     self._predictions['text_pred'].data.cpu().numpy()
    return cls_score, cls_prob, bbox_pred, rois, text_pred
  ...
  def train_step(self, blobs, train_op):
    ...
    self.forward(blobs['data'], blobs['im_info'], blobs['gt_boxes'],
                    blobs['gt_texts'], blobs['gt_pair'])
    rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_text,loss_pair,loss = self._losses["rpn_cross_entropy"].data[0], \
                                                                        self._losses['rpn_loss_box'].data[0], \
                                                                        self._losses['cross_entropy'].data[0], \
                                                                        self._losses['loss_box'].data[0], \
                                                                        self._losses['loss_text'].data[0], \
                                                                        self._losses['loss_pair'].data[0], \
    ...
    return rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_text,loss_pair, loss
  ...
  def train_step_with_summary(self, blobs, train_op):
    ...
    return rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_text,loss_pair, loss, summary
  ...
  def load_state_dict_crnn(self, state_dict):
    """
