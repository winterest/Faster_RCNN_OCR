...
class SolverWrapper(object):
  ...
  def construct_graph(self):
    ...
    for key, value in dict(self.net.named_parameters()).items():
      if value.requires_grad:
        ...
        print(key)
        #params += [{'params':[value],'lr':lr, 'weight_decay': cfg.TRAIN.WEIGHT_DECAY}]
    self.optimizer = torch.optim.Adam(params,lr=0.00001,betas=(0.5,0.999))
    #self.optimizer = torch.optim.Adam(self.net.parameters(),lr=0.001,betas=(0.5,0.999))
  ...
  def initialize(self):
    ...
    self.net.load_pretrained_cnn(torch.load(self.pretrained_model, map_location=lambda storage, loc:storage))
    print('Loaded.')
  ...
  def train_model(self, max_iters):
    ...
    while iter < max_iters + 1:
      self.net._alp_pair = 0
      #'''
      if (iter>10001):
        #self.net._alp_pair = 1
        self.net._alp_text = 1
      #'''
      # Learning rate
      if (iter == max_iters//2) or (iter==10001):
        # release training for OCR
        for param in self.net.crnn_net.parameters():
            print("begin training OCR")
      ...
      if iter == 1 or now - last_summary_time > cfg.TRAIN.SUMMARY_INTERVAL:
        ...
        rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_text, loss_pair, total_loss, summary = \
        ...
        rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_text,  loss_pair, total_loss = \
      ...
      if iter % (cfg.TRAIN.DISPLAY) == 0:
        print('iter: %d / %d, total loss: %.6f\n >>> rpn_loss_cls: %.6f\n '
              '>>> rpn_loss_box: %.6f\n >>> loss_cls: %.6f\n >>> loss_box: %.6f\n >>> loss_text: %.6f\n >>> loss_pair: %.6f\n >>> lr: %f' % \
              (iter, max_iters, total_loss, rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_text,loss_pair, lr))
